...
if ($method == 'h_error_chart') {
   ...
   $check = "SELECT DATE_FORMAT(error_date, '%H') AS hour_of_error, COUNT(*) AS count_per_hour, category
   FROM sam_error
   WHERE error_date LIKE '$h_date%'
     AND sam_machine = '$h_machine_list'
     AND (category LIKE '$h_category_list%' OR category LIKE '$h_category_list2%')
   GROUP BY hour_of_error;
   ";
